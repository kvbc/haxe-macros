				...
				for (localField in fields) {
					if (localField.name == baseField.name) {
						...
						if (hasMeta(localField.meta, forwardFromBaseMetaName)) {
							...
							Context.error('local class "$localID": field "${localField.name}" has the same name as base field in base class "${baseID}"',
								localField.pos);
						}
						break;
					}
				...
				var baseRet;
				{
					...
					switch (type) {
						case TFun(args, ret):
							...
							baseRet = ret;
						case _:
							throw "Error"; // static fields with @:op should always be functions
					}
				}

				var newExpr:Expr = wrapExpr({
					var castedThis:Expr = wrapExpr(ECast(wrapExpr(EConst(CIdent("this"))), baseComplexType));

					switch (baseField.meta.extract(':op')[0].params[0].expr) {
						case EBinop(op, _, _):
							EBinop(op, castedThis, wrapExpr(ECast(wrapExpr(EConst(CIdent(baseParams[1].name))), Context.toComplexType(baseParams[1].t))));
					...
					args: (baseParams.length == 1) ? [] : [
						{
							...
							type: (TypeTools.toString(baseParams[1].t) == baseID) ? localComplexType : Context.toComplexType(baseParams[1].t) // value?
								// meta?
						}
					],
					// ret: (getAbstractRef(baseRet).toString() == baseAbstractRef.toString())
					ret: (TypeTools.toString(baseRet) == baseID) ? localComplexType : Context.toComplexType(baseRet),
